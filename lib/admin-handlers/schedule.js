...
async function handleFree(req, res) {
  ...
  if (body.include_drafts === true) {
    // SECUENCIAL a propósito, no Promise.all: cada asignación cambia el "este
    // coche no ha salido nunca" del siguiente día. En paralelo, varios días
    // podrían llevarse el mismo coche del tema.
    for (const date of dates) {
      const allowDrafts = draftsAllowedFor({ date, today });
      const { error: rpcErr } = await getSupabaseAdmin().rpc("pick_daily_car", {
        p_date: date,
        p_allow_drafts: allowDrafts,
      });
      if (rpcErr) {
        // No abortamos: un día sin asignar lo rellenará el GET con un coche con
        // foto. Peor sería dejar la mitad del calendario liberada y sin sortear.
