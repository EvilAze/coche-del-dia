...
// Umami Cloud API (para "uso del ranking"). Base + header documentados en
// https://docs.umami.is. El website-id es PÚBLICO (está en index.html), así
// que un fallback hardcodeado es seguro; permitimos override por env.
const UMAMI_API_BASE = "https://api.umami.is/v1";
const UMAMI_WEBSITE_ID =
  process.env.UMAMI_WEBSITE_ID || "6d3e8cc0-401e-42b0-8cd4-dfc2a2748f1b";
...
async function fetchUsers(supabaseAdmin, fromIso) {
  ...
  if (error) {
    ...
    return { total: 0, newInPeriod: 0, directory: [], all: [] };
  }
  ...
  // último login desc; los que nunca iniciaron sesión caen al final.
...
// ---------- Bloque 10: Uso del ranking (evento Umami) -----------------

// Lee de la API de Umami Cloud cuántas veces se abrió el ranking (evento
// custom `ranking_open`, disparado en App.jsx) y lo relaciona con las visitas
// del mismo periodo para responder "¿se usa de verdad esta palanca?".
//
// NO añade función serverless: corre dentro de este handler admin, que ya
// existe. Best-effort total: si falta UMAMI_API_KEY o Umami no responde,
// devolvemos {configured:false|error:true} y el panel lo indica sin romperse.
// Solo 2 fetches por carga (límite Umami: 50 req/15s — de sobra).
async function fetchRankingUsage(fromIso) {
  const key = process.env.UMAMI_API_KEY;
  if (!key) return { configured: false };

  const startAt = new Date(`${fromIso}T00:00:00Z`).getTime();
  const endAt = Date.now();
  const headers = { Accept: "application/json", "x-umami-api-key": key };
  const base = `${UMAMI_API_BASE}/websites/${UMAMI_WEBSITE_ID}`;

  try {
    const [statsRes, eventsRes] = await Promise.all([
      fetch(`${base}/stats?startAt=${startAt}&endAt=${endAt}`, { headers }),
      fetch(`${base}/metrics?type=event&startAt=${startAt}&endAt=${endAt}`, { headers }),
    ]);
    if (!statsRes.ok || !eventsRes.ok) {
      console.error("[admin/analytics] umami http:", statsRes.status, eventsRes.status);
      return { configured: true, error: true };
    }
    const stats = await statsRes.json();
    const events = await eventsRes.json();

    // Umami devuelve, según versión, números planos o {value}. Normalizamos.
    ...
    const row = Array.isArray(events) ? events.find((e) => e.x === "ranking_open") : null;
    const pulsaciones = row ? Number(row.y) || 0 : 0;
    ...
    return {
      ...
      visitors,
      pageviews,
      // Proxy de "% de uso": aperturas sobre visitas del periodo. Son
      // pulsaciones TOTALES (no únicos): puede superar el 100% si la gente
      // abre el ranking varias veces por visita. El panel lo etiqueta así.
      perVisit: visits > 0 ? pulsaciones / visits : null,
    };
  } catch (err) {
    console.error("[admin/analytics] umami fetch:", err?.message || err);
    return { configured: true, error: true };
  }
}
