...
async function handleGet(req, res) {
  const { data, error } = await getSupabaseAdmin()
    .from("seasons")
    .select("id, number, label_es, label_en, presenta_es, presenta_en, starts_at, ends_at, closed_at, theme_filter")
...
async function handleUpsert(req, res) {
  ...
  const row = {
    ...
    // Quién presenta la temporada, si viene de una colaboración. Cadena vacía
    // → null y no "": una temporada sin patrocinio no tiene que distinguirse de
    // otra a la que le borraron el patrocinio, y el cliente solo comprueba si
    // hay algo que pintar.
    presenta_es: cleanStr(body.presenta_es) || null,
    presenta_en: cleanStr(body.presenta_en) || null,
