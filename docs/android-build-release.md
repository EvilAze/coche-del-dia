...
| `plugins.SystemBars.insetsHandling` | `disable` | **Es lo que impide que el teclado encoja el WebView.** Quien lo encogía no era `adjustResize` (con targetSdk 36 se ignora) ni `@capacitor/keyboard` (su `resize` es *iOS only*; en Android `setResizeMode` es `call.unimplemented()`): era el propio Capacitor, en `SystemBars.java` → `initWindowInsetsListener()`, con un `v.setPadding(0, 0, 0, keyboardVisible ? imeInsets.bottom : 0)` sobre el padre del WebView. Eso hacía que al subir el teclado la ventana pasara de 780 a 490, el pliego se recompusiera y la fotografía bajara de 336x252 a 232x174 — encogiendo además una cantidad distinta en cada móvil. Con `disable` el oyente no se instala. **Contrapartida, y resultó tener DOS mitades — las dos las cubre ahora `InsetsBridgePlugin` (`android/app/src/main/java/com/cochedeldia/`), que escucha `WindowInsets` y publica variables CSS SIN tocar el tamaño de ninguna vista:** (1) apagar el oyente apaga también `injectSafeAreaCSS()`, y aquí se dio por hecho que el `env()` del navegador lo sostendría — no lo sostiene: el WebView de Android mide bien `env(safe-area-inset-top)` (53px) y devuelve **0** en `bottom` por debajo de Chromium 140, que es por lo que la última fila de la lista y el reloj de cierre se dibujaban bajo la barra de gestos. La forma canónica en `index.css` es `var(--safe-area-inset-X, env(...))`, con la variable primero. (2) Sin el redimensionado, el teclado **se superpone** al WebView: `innerHeight` y `visualViewport.height` siguen midiendo la pantalla entera con él subido, así que ningún `dvh` se entera y la hoja de selección se dibujaba debajo. Su alto va por `--cdd-teclado` (más `data-teclado-sistema` y el evento `cdd:teclado`), **aparte** de los cuatro insets: sumárselo a `--safe-area-inset-bottom` movería el pliego entero y reintroduciría por la puerta de atrás justo lo que `disable` vino a quitar. Si algún día el contenido aparece bajo la barra de estado o bajo el teclado, empieza mirando aquí y pasa `npm run test:layout`. Marcha atrás: quitar la clave. |
