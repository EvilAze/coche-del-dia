...
| `plugins.SystemBars.insetsHandling` | `disable` | **Es lo que impide que el teclado encoja el WebView.** Quien lo encogía no era `adjustResize` (con targetSdk 36 se ignora) ni `@capacitor/keyboard` (su `resize` es *iOS only*; en Android `setResizeMode` es `call.unimplemented()`): era el propio Capacitor, en `SystemBars.java` → `initWindowInsetsListener()`, con un `v.setPadding(0, 0, 0, keyboardVisible ? imeInsets.bottom : 0)` sobre el padre del WebView. Eso hacía que al subir el teclado la ventana pasara de 780 a 490, el pliego se recompusiera y la fotografía bajara de 336x252 a 232x174 — encogiendo además una cantidad distinta en cada móvil. Con `disable` el oyente no se instala. **Contrapartida a vigilar:** apaga también `injectSafeAreaCSS()`, que publica `--safe-area-inset-*`; los 14 usos de `index.css` están escritos como `var(--safe-area-inset-top, env(safe-area-inset-top, 0px))` e `index.html` lleva `viewport-fit=cover`, así que el respaldo nativo debería sostenerlos. Si algún día el contenido aparece bajo la barra de estado, empieza mirando aquí. Marcha atrás: quitar la clave. |
