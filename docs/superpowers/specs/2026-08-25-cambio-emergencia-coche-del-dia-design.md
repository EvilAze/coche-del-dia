...
| **R4b** | Y algo más fino que se ve al mirar el `v`: quien jugara el coche saliente durante la ventana de emergencia pudo quedarse con su `v`, que es estable mientras no cambien foto ni `zoom_base`. El día que ese coche vuelva a salir, **lo reconoce en el intento 1** comparando el `v` de la URL, sin esperar a que la foto revele nada | No es una fuga nueva —esa persona ya vio la foto ese día y lo reconocería igual—, pero sí adelanta el momento. Afecta solo a quien jugó en esa ventana. Si en algún caso importa, basta con **cambiar la foto o el `zoom_base` del coche saliente** antes de que vuelva: el `v` deriva de los dos, así que cambia con ellos |
...
| **R6** | La RPC nueva está en el **camino crítico del primer paint** | Es un envoltorio que no toca `pick_daily_car`, y el handler cae a `pick_daily_car` si falla — pero **solo cuando el fallo fue instantáneo**: `coche_de_hoy` llama por dentro a `pick_daily_car`, así que detrás de una espera agotada el respaldo no arregla nada y sí gasta otro plazo (→ 503 con `Retry-After`). Y el respaldo vive DENTRO de la fase paralela, compitiendo con auth en un `max()` en vez de sumándose detrás. El test que suma la cadena de plazos (regla 21) cubre **las dos** cadenas, con respaldo y sin él, contra los 25 s del Edge |
