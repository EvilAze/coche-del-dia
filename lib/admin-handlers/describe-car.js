...
// Extrae el texto limpio de la respuesta de Gemini.
function extraerTexto(response) {
  ...
  // Si Gemini lo envolvió en un bloque de código markdown
  texto = texto.replace(/^```(?:json|text)?\n?/i, "").replace(/```$/i, "").trim();

  // Si intentó devolver JSON a pesar de no pedir schema
  if (texto.startsWith("{") && texto.endsWith("}")) {
    try {
      const parsed = JSON.parse(texto);
      if (parsed.descripcion) texto = parsed.descripcion;
      else if (parsed.text) texto = parsed.text;
    } catch (e) {}
  }

  // Limpiar prefijos basura que a veces añade como "text*:" o "**Descripción:** "
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Import dinámico: si @google/genai no se puede resolver, solo muere ESTE
    // endpoint — no arrastra al dispatcher entero (que comparte serverless con
    // save-car, analytics, etc.). Antes era un import estático a nivel de
    // módulo y un fallo del SDK tumbaba todo el panel admin.
    const { GoogleGenAI } = await import("@google/genai");
    const ai = new GoogleGenAI({ apiKey });
    const model = process.env.DESCRIBE_MODEL || DEFAULT_MODEL;

    // Intento 1: con Google Search grounding (datos verificados con la web).
    // Si la cuota de búsqueda está agotada (429 / RESOURCE_EXHAUSTED), caemos
    // al intento 2 sin búsqueda: el modelo tira de memoria, menos fiable pero
    // mejor que nada — el admin siempre revisa antes de guardar.
    let response;
    let usedSearch = true;
    try {
      ...
      const searchMsg = searchErr?.message || String(searchErr);
      const isQuota = searchMsg.includes("429") || searchMsg.includes("RESOURCE_EXHAUSTED");
      if (!isQuota) throw searchErr; // Error real, que suba al catch global

      // Fallback sin búsqueda web
      console.warn("[admin/describe-car] Google Search agotado, fallback sin búsqueda");
      usedSearch = false;
      response = await ai.models.generateContent({
        model,
        contents: buildUserPrompt({ marca, modelo, anio, pais }),
    ...
    return res.status(200).json({
      descripcion,
      model,
      // El front puede mostrar un aviso si se generó sin búsqueda web
      ...(usedSearch ? {} : { warning: "Generada sin búsqueda web (cuota agotada). Revisa los datos con especial atención." }),
    });
  } catch (err) {
