...
async function computeSuspects(admin, fromIso, emailById) {
  ...
  const { data: dailies, error: dErr } = await dq;
  if (dErr) {
  ...
  const { data: rows, error: gErr } = await gq;
  if (gErr) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const suspects = await computeSuspects(admin, fromIso, emailById);
    ...
    const { data: auditRows, error: rowsErr } = await q;
    ...
    if (rowsErr) {
      ...
      const guessRows = auditRows.filter((r) => r.mode !== "canary");
