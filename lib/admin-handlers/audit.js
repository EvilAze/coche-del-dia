...
// Nicks (id -> display_name) desde public.profiles.
//
// El correo identifica la CUENTA, pero el nick es lo único que se ve en la
// clasificación: sin él, «este de aquí hace 90% de aciertos en frío» y «este
// nombre que lleva tres semanas primero» son dos hechos que el admin no puede
// juntar sin salir del panel. Va con service_role, así que se salta el RLS y ve
// también los perfiles ya marcados (`profiles_select` los esconde).
//
// Si falla, se sigue: un panel de auditoría sin nicks es peor, pero un panel de
// auditoría que no carga es inútil. Mismo criterio que el directorio de
// analytics.js.
async function loadNicks(admin) {
  const nickById = new Map();
  const { data, error } = await admin.from("profiles").select("id, display_name");
  if (error) {
    console.warn("[admin/audit] nicks no disponibles:", error.message);
    return nickById;
  }
  for (const p of data || []) {
    if (p.display_name) nickById.set(p.id, p.display_name);
  }
  return nickById;
}

// ---------- Sección 1: ranking de sospecha (user_guesses daily) --------
async function computeSuspects(admin, fromIso, emailById, nickById) {
  // Mapa date -> coches del daily, para aislar partidas daily (las repescas
  // llevan un car_id que no fue el del día y contaminarían: el veterano gana en
  // 1 intento sabiendo la respuesta legítimamente).
  ...
  // date -> Set de TODOS los coches que fueron el del día en esa fecha.
  ...
  for (const [userId, u] of byUser) {
    ...
    suspects.push({
      ...
      username: nickById?.get(userId) || null,
      userId,
      games: u.n,
      wins: u.wins,
      losses: u.n - u.wins,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Las dos caras de una cuenta: el correo (con el que se actúa) y el nick
    // (con el que se la ve en el juego). En paralelo porque son dos lecturas
    // independientes y el panel ya tarda lo suyo.
    const [emailById, nickById] = await Promise.all([
      loadEmails(admin),
      loadNicks(admin),
    ]);

    // Nombre corto para las líneas sueltas (timeline, canarios), donde no cabe
    // la ficha de dos renglones: manda el nick, y el correo viaja aparte para
    // que el front lo ponga en el `title`.
    const quien = (userId) =>
      nickById.get(userId) || emailById.get(userId) || userId;

    // --- Sección 1: ranking de sospecha (independiente de guess_audit) ---
    const { suspects, population } = await computeSuspects(
      admin,
      fromIso,
      emailById,
      nickById
    ...
    // interfaz.
    //
    // Sale de `profiles.is_flagged`, que es el shadowban del proyecto desde
    // junio de 2026 (ver scripts/2026-08-unificar-shadowban.sql). Y ojo con el
    // detalle que lo hace legible desde aquí: esta consulta va con
    // service_role, que se salta RLS. Con la anon key no se vería ninguna de
    // estas filas — precisamente porque `profiles_select` las esconde.
    let excluidos = [];
    {
      const { data, error } = await admin
        ...
        .eq("is_flagged", true);
      if (error) {
        console.warn("[admin/audit] is_flagged no disponible:", error.message);
      } else {
        excluidos = (data || []).map((r) => r.id);
      }
    }
      ...
      flags = rawFlags
        ...
        .map((f) => {
          ...
          return {
            ...
            winnerNick: nickById.get(f.winnerId) || null,
            timeline: f.list.map((r) => ({
              ts: r.ts,
              mode: r.mode,
              who: r.is_anon ? "ANÓNIMO" : quien(r.user_id),
              whoEmail: r.is_anon ? null : (emailById.get(r.user_id) || null),
              isAnon: r.is_anon,
