  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const { data, error } = await getSupabaseAdmin()
        .from("cars")
        .select("id, make, model, year, pais, description, description_en, image_url, focus_x, focus_y, image_ready")
        .eq("id", idQ)
        .maybeSingle();
      if (error) {
    ...
    const { data, error } = await getSupabaseAdmin()
      .from("cars")
      .insert(insertRow)
      .select("id, make, model, year, pais, description, description_en, image_url, image_ready")
      .maybeSingle();
