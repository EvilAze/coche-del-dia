...
// Registrados que COMPLETARON el daily (status won/lost), distintos por día.
...
// no la del DAU.
async function fetchRegisteredFinishedSeries(supabaseAdmin, fromIso, toIso) {
  const { data, error } = await supabaseAdmin
    .from("user_guesses")
    .select("user_id, date")
    .in("status", ["won", "lost"])
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [
      ...
      fetchRegisteredFinishedSeries(supabaseAdmin, fromIso, toIso),
