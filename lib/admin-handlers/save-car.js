...
//     tags?:           string[]                   // etiquetas de temporada temática
//     video_id?:       string                     // ID o URL de YouTube; "" lo borra
...
//
// `tags` viaja SOLO por aquí (service_role). La columna no tiene GRANT para
// anon/authenticated a propósito: si el cliente pudiera leerla, cruzaría el
// catálogo con la temática de la temporada y acotaría el coche del día a la
// lista exacta de candidatos (ver scripts/2026-07-temporadas-tematicas.sql).
//
// `video_id` va en el mismo saco y por partida doble: un ID de YouTube nombra
// el coche del día, y además estará poblado justo en los coches de la
// temporada, así que leerlo sería leer el pool (ver
// scripts/2026-08-temporada-presentada-y-video.sql). Al jugador solo le llega
// dentro del `reveal` de una partida ya cerrada.

import { generateBlurData } from "../../api/_lib/blur-data.js";
...
import { normalizeCarTags } from "../../api/_lib/season-theme.js";
import { normalizeVideoId } from "../../api/_lib/video-id.js";
import {
  DEFAULT_ZOOM_BASE,
  ZOOM_BASE_MIN,
  ZOOM_BASE_MAX,
...
function shapeCarResponse(row) {
  return {
    ...
    // Zoom inicial del intento 1 (dificultad). Default si la fila es anterior
    // a la columna.
    zoom_base:
      typeof row.zoom_base === "number" ? row.zoom_base : DEFAULT_ZOOM_BASE,
    // Flag de "listo para salir como coche del día". FALSE = el coche existe
    // en catálogo (autocomplete del juego, garaje, etc.) pero pick_daily_car
    // lo ignora hasta que tenga imagen subida. Se flip a TRUE automáticamente
    // en el update cuando llega un image_url válido.
    image_ready: row.image_ready !== false,
    // Etiquetas para las Temporadas Temáticas. Array vacío en filas anteriores
    // a la columna (DEFAULT '{}') y también si la query no la pidió.
    tags: Array.isArray(row.tags) ? row.tags : [],
    // Vídeo del coche (temporadas presentadas). Sale por AQUÍ y por el `reveal`
    // de una partida cerrada, y por ningún sitio más: este endpoint es admin y
    // va con service_role, así que la columna sin GRANT no le afecta.
    video_id: row.video_id ?? null,
  };
}

// Intel de dificultad observada (DDA Arquitectura A). Solo la rellena el GET:
// las columnas las computa recompute_car_difficulty desde la telemetría. null
// si el coche aún no tiene datos medidos (nunca fue coche del día, o muy pocas
// partidas). El admin la usa para decidir si aplica suggested_zoom_base.
function shapeDifficulty(row) {
  if (!row || typeof row.difficulty_n !== "number") return null;
  return {
    n: row.difficulty_n,
    cost: typeof row.difficulty_cost === "number" ? row.difficulty_cost : null,
    costShrunk:
      typeof row.difficulty_cost_shrunk === "number" ? row.difficulty_cost_shrunk : null,
    pBy3: typeof row.difficulty_p_by_3 === "number" ? row.difficulty_p_by_3 : null,
    failRate: typeof row.difficulty_fail_rate === "number" ? row.difficulty_fail_rate : null,
    suggestedZoomBase:
      typeof row.suggested_zoom_base === "number" ? row.suggested_zoom_base : null,
    computedAt: row.difficulty_computed_at ?? null,
  };
}
...
// Validador de zoom_base (zoom inicial del coche). Mismo patrón que parseFocus:
// null = "no presente" (no tocar la columna / dejar el default de la BD),
// undefined = "inválido" → 400, número = valor saneado dentro de rango.
function parseZoomBase(value) {
  if (value === undefined || value === null) return null;
  const n = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(n)) return undefined;
  if (n < ZOOM_BASE_MIN || n > ZOOM_BASE_MAX) return undefined;
  return n;
}
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const { data, error } = await getSupabaseAdmin()
        .from("cars")
        .select("id, make, model, year, pais, description, description_en, image_url, focus_x, focus_y, zoom_base, image_ready, tags, video_id, difficulty_n, difficulty_cost, difficulty_cost_shrunk, difficulty_p_by_3, difficulty_fail_rate, suggested_zoom_base, difficulty_computed_at")
        .eq("id", idQ)
        .maybeSingle();
      if (error) {
      ...
      return res.status(200).json({ ...shapeCarResponse(data), difficulty: shapeDifficulty(data) });
    }
    ...
    if (isUpdate) {
      ...
      if (typeof body.image_url === "string" && body.image_url.startsWith("http")) {
        ...
        // Auto-flip a "listo para pick_daily_car". Esto es la pieza clave
        // del flujo "alta sin imagen": el admin mete el coche en SQL/draft
        // (image_ready=FALSE) y al subirle la imagen aquí, queda elegible
        // para salir como coche del día sin tocar el RPC ni nada más.
        patch.image_ready = true;
      }
      ...
      // zoom_base — zoom inicial / dificultad del coche.
      if ("zoom_base" in body) {
        const zb = parseZoomBase(body.zoom_base);
        if (zb === undefined) {
          return res
            .status(400)
            .json({ error: `zoom_base fuera de [${ZOOM_BASE_MIN}, ${ZOOM_BASE_MAX}]` });
        }
        if (zb !== null) patch.zoom_base = zb;
      }
      // tags — etiquetas de temporada temática. Aquí `[]` SÍ se escribe (es
      // "quítale todas"), a diferencia del resto de campos donde vacío = no
      // tocar; por eso el guard es `in body` y no un truthy.
      if ("tags" in body) {
        const { value: tags, error: tagsError } = normalizeCarTags(body.tags);
        if (tagsError) return res.status(400).json({ error: tagsError });
        if (tags !== null) patch.tags = tags;
      }
      // video_id — el vídeo del coche en las temporadas presentadas. Mismo
      // guard `in body` que tags y por el mismo motivo: aquí la cadena vacía SÍ
      // se escribe, porque es la única forma de QUITARLE el vídeo a un coche
      // (con el criterio general de "vacío = no tocar", una vez puesto no habría
      // manera de retirarlo desde el panel).
      if ("video_id" in body) {
        const { value: vid, error: vidError } = normalizeVideoId(body.video_id);
        if (vidError) return res.status(400).json({ error: vidError });
        patch.video_id = vid;
      }
      ...
      const { data, error } = await getSupabaseAdmin()
        ...
        .update(patch)
        .eq("id", idRaw)
        .select("id, make, model, year, pais, description, description_en, image_url, focus_x, focus_y, zoom_base, image_ready, tags, video_id")
        .maybeSingle();
      if (error) {
    ...
    // zoom_base opcional en el alta. Si no viene, la BD aplica el DEFAULT 3.7
    // → "el nivel de zoom que tenemos ahora", y el admin lo ajusta luego.
    const zoomBaseIn = parseZoomBase(body.zoom_base);
    if (zoomBaseIn === undefined) {
      return res
        .status(400)
        .json({ error: `zoom_base fuera de [${ZOOM_BASE_MIN}, ${ZOOM_BASE_MAX}]` });
    }

    // tags opcionales en el alta. Si no vienen, la BD aplica el DEFAULT '{}'.
    const { value: tagsIn, error: tagsError } = normalizeCarTags(body.tags);
    if (tagsError) return res.status(400).json({ error: tagsError });
    ...
    if (zoomBaseIn !== null) insertRow.zoom_base = zoomBaseIn;
    if (tagsIn !== null) insertRow.tags = tagsIn;
