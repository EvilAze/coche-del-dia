...
//     Mete o saca la cuenta de `excluidos_de_clasificacion`, que es lo que
//     miran los tres embudos del ranking (ver
//     scripts/2026-08-exclusion-de-clasificacion.sql). Sigue jugando, sigue
//     sumando puntos y sigue viendo sus estadísticas; simplemente no sale en
//     ninguna tabla pública ni entra en los podios que se sellen luego.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (accion === "excluir-clasificacion") {
      // upsert: excluir a quien ya está excluido no es un error, es un no-op.
      const { error } = await admin.from("excluidos_de_clasificacion").upsert(
        {
          user_id: userId,
          motivo: motivo || "patrón de oráculo en el panel de Auditoría",
          por: user?.email || "admin",
        },
        { onConflict: "user_id" }
      );
      if (error) {
        console.error("[admin/moderacion] excluir:", error.message);
        return res.status(500).json({ error: "DB error" });
      }
      console.log(
        `[admin/moderacion] ${user?.email || "?"} excluyó de la clasificación a ${userId}`
      );
      return res.status(200).json({ ok: true, excluido: true });
    }

    if (accion === "readmitir-clasificacion") {
      const { error } = await admin
        .from("excluidos_de_clasificacion")
        .delete()
        .eq("user_id", userId);
      if (error) {
        console.error("[admin/moderacion] readmitir:", error.message);
        return res.status(500).json({ error: "DB error" });
      }
      console.log(
        `[admin/moderacion] ${user?.email || "?"} readmitió a ${userId}`
      );
      return res.status(200).json({ ok: true, excluido: false });
    }
