  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const { data, error } = await getSupabaseAdmin()
        .from("cars")
        .select("id, make, model, year, pais, description, description_en, image_url, focus_x, focus_y, zoom_base, image_ready, difficulty_n, difficulty_cost, difficulty_cost_shrunk, difficulty_p_by_3, difficulty_fail_rate, suggested_zoom_base, difficulty_computed_at")
        .eq("id", idQ)
        .maybeSingle();
      if (error) {
    ...
    if (isUpdate) {
      ...
      const { data, error } = await getSupabaseAdmin()
        ...
        .update(patch)
        .eq("id", idRaw)
        .select("id, make, model, year, pais, description, description_en, image_url, focus_x, focus_y, zoom_base, image_ready")
        .maybeSingle();
      if (error) {
