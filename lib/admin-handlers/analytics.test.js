...
import { clasificarRepescas, fetchPlataformas, fetchUsers, serieDesdeMapa } from "./analytics.js";
...
// ---------------------------------------------------------------------------
// fetchUsers: la paginación de auth.users
// ---------------------------------------------------------------------------
// Por qué existe: `listUsers` corta en 1000 por página y antes sólo se pedía la
// primera. No fallaba — devolvía menos filas en silencio. Y el techo no lo
// marcan los registrados sino que auth.users guarda TAMBIÉN las sesiones
// anónimas (una por navegador que juega), que se acumulan hasta que pasa el
// cron de limpieza. O sea que el corte se alcanza por el lado que nadie mira, y
// cuando se alcance congela a la vez «Usuarios registrados» y «Sesiones
// anónimas».

// Servidor falso de auth.admin: reparte `todos` en páginas de `porPagina`.
function authFalso(todos, { porPagina = 1000, fallaEnPagina = null } = {}) {
  const llamadas = [];
  return {
    llamadas,
    auth: {
      admin: {
        listUsers: async ({ page, perPage }) => {
          llamadas.push(page);
          if (fallaEnPagina === page) {
            return { data: null, error: { message: "boom" } };
          }
          const tam = Math.min(perPage, porPagina);
          const desde = (page - 1) * tam;
          return { data: { users: todos.slice(desde, desde + tam) }, error: null };
        },
      },
    },
    // fetchUsers también lee public.profiles para los nicks.
...
const cuenta = (i, anon) => ({
  id: `u${i}`,
  email: anon ? null : `u${i}@ejemplo.com`,
  is_anonymous: anon,
  created_at: "2026-08-01T00:00:00Z",
  last_sign_in_at: "2026-08-10T00:00:00Z",
});

describe("fetchUsers · paginación", () => {
  it("una sola página: no pide una segunda si el lote viene incompleto", async () => {
    const api = authFalso([cuenta(1, false), cuenta(2, false)]);
    const r = await fetchUsers(api, "2026-08-01");
    expect(r.total).toBe(2);
    expect(api.llamadas).toEqual([1]);
  });

  it("pasadas las 1000 filas sigue paginando en vez de truncar en silencio", async () => {
    // 1200 anónimas + 30 reales = 1230 filas, o sea dos páginas completas.
    const todos = [
      ...Array.from({ length: 1200 }, (_, i) => cuenta(i, true)),
      ...Array.from({ length: 30 }, (_, i) => cuenta(2000 + i, false)),
    ];
    const r = await fetchUsers(authFalso(todos), "2026-08-01");
    // Con el corte viejo esto daba 0 registrados y 1000 anónimas: las 30
    // cuentas reales caían enteras fuera de la primera página.
    expect(r.total).toBe(30);
    expect(r.anonTotal).toBe(1200);
    expect(r.directory).toHaveLength(30);
  });

  it("si falla la PRIMERA página se rinde a cero (no hay nada que enseñar)", async () => {
    const api = authFalso([cuenta(1, false)], { fallaEnPagina: 1 });
    const r = await fetchUsers(api, "2026-08-01");
    expect(r).toEqual({
      total: 0, newInPeriod: 0, directory: [], anonTotal: 0, anonNewInPeriod: 0,
    });
  });

  it("si falla una página POSTERIOR conserva lo ya leído", async () => {
    const todos = Array.from({ length: 1500 }, (_, i) => cuenta(i, false));
    const r = await fetchUsers(authFalso(todos, { fallaEnPagina: 2 }), "2026-08-01");
