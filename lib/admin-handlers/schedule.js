...
//     Asigna `car_id` al día `date` (futuro estricto, dentro de la ventana
//     de 14 días — a HOY se llega solo por /api/admin/emergency-swap). Si ese
//     coche estaba en otra fecha futura, libera la vieja. Si ya salió en el
//     pasado o es el coche de hoy, rechaza con 409 — no permitimos reusar un
//     coche ya jugado.
//
...
async function handlePost(req, res) {
  ...
  // Solo permitimos tocar fechas ESTRICTAMENTE FUTURAS dentro de la ventana
  // visible. Bloquear el pasado es obvio (ya se jugó), y bloquear más allá de
  // hoy+13 evita asignaciones "fantasma" que el admin no recordaría.
  //
  // HOY tampoco: antes este POST lo aceptaba y lo único que lo impedía era un
  // botón deshabilitado en el panel — la cerradura era de cortesía. Cambiar el
  // coche del día con partidas en curso tiene su propia puerta
  // (/api/admin/emergency-swap), que avisa a cuánta gente afecta y guarda el
  // coche saliente en daily_cars.prev_car_ids. El guard vive en
  // api/_lib/schedule-free.js, con tests, por lo mismo que el de liberar.
  const today = todayInMadrid();
  const maxDate = addDays(today, DAYS_WINDOW - 1);
